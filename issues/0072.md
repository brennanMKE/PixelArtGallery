...
> 🟢 Resolved 2026-07-11 — Reviewed `a0d9a38`: the render/layout split that caused the black bar is structurally gone (`.ignoresSafeArea(edges:.top)` now scopes only to a fixed-size 220pt `BackgroundPixelsView(style:.vibrant)` background container, so the header's layout stays in the safe area and the pixel grid can't re-randomize mid-collapse), the gallery is now one full-height `ScrollView` with the header as a top `.overlay` fed by `onScrollGeometryChange` + a constant `.contentMargins(.top, 128)` inset (no separate-ScrollView edge to clip the top row), and `GalleryBottomBar` drops ~86pt→52pt (−40%; `+` circle 60→44pt, raise offset removed) while keeping ≥44pt tap targets. `GalleryHeaderMetrics` is a `nonisolated` pure helper, clamped 0…1 and monotonic. Independently verified: `swift test` 188 tests pass incl. all 6 new `GalleryHeaderMetricsTests` by name; macOS + iOS Simulator builds both succeed with no type-check-time error (the SourceKit `:183` complaint was a stale-index false positive); at-rest sim screenshots (light + dark) confirm the black bar is gone and the bar is shorter. Residual risk: the collapse-*on-scroll* / pin / no-clip-while-scrolling behavior could not be driven headlessly and remains a device-confirmation step.
...
| 2026-07-11 | review | claude-opus-4-8 | 8,335 | 149 | 383,816 | 62,008 | $0.62 |
